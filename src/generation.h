        ...
        struct ExprVisitor {
            ...
            void operator()(const NodeExprIdent& expr_ident) {
                if (!gen->m_vars.contains(expr_ident.ident.value.value())) {
                    std::cout << "Undeclared Identifier" <<expr_ident.ident.value.value() << std::endl;
                    exit(EXIT_FAILURE);
                }
                const auto& var = gen->m_vars[expr_ident.ident.value.value()];
                std::stringstream offset;
                offset << "QWORD [rsp + " + std::to_string((gen->m_stack_size - var.stack_loc - 1) * 8) + "]\n";
                gen->push(offset.str());
            }
        };
        ...
        struct StmtVisitor {
            ...
            void operator()(const NodeStmtLet& stmt_let) {
                if (gen->m_vars.contains(stmt_let.ident.value.value())) {
                    std::cout << "Identifier Already Used: " << stmt_let.ident.value.value() << std::endl;
                    exit(EXIT_FAILURE);
                }
